...
@Service
public class DecideIfSubmissionNeedsBetaReadersCreateListener implements TaskListener {
    @Override
    public void notify(DelegateTask delegateTask) {
        ...
        FormField field = formFields.stream().filter(formField -> formField.getId().equals("needsBetareaders")).collect(Collectors.toList()).get(0);
