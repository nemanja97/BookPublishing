...
@Service
public class DecideOnPublishingCreateListener implements TaskListener {
    @Override
    public void notify(DelegateTask delegateTask) {
        ...
        FormField field = formFields.stream().filter(formField -> formField.getId().equals("shouldBePublished")).collect(Collectors.toList()).get(0);
