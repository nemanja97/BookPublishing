...
@Service
public class DecideIfSubmissionIsPlagiarisedCreateListener implements TaskListener {
    @Override
    public void notify(DelegateTask delegateTask) {
        ...
        FormField field = formFields.stream().filter(formField -> formField.getId().equals("plagiarism")).collect(Collectors.toList()).get(0);
