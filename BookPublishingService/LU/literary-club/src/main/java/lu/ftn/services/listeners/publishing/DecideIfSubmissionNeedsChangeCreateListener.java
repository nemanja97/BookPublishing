...
@Service
public class DecideIfSubmissionNeedsChangeCreateListener implements TaskListener {
    @Override
    public void notify(DelegateTask delegateTask) {
        ...
        FormField field = formFields.stream().filter(formField -> formField.getId().equals("needsChanges")).collect(Collectors.toList()).get(0);
